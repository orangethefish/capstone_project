...
for k in range(1):
    fileLength = 0
    input_file = f"L_{k}"  # Replace with the actual file path
    with open(f"{input_file}.csv", 'r') as csv_file:
        ...
        for data in csv_reader:
            accel_x_data.append(float(data['ax']))
            accel_y_data.append(float(data['ay']))
            accel_z_data.append(float(data['az']))
            gyro_x_data.append(float(data['gx']))
            gyro_y_data.append(float(data['gy']))
            gyro_z_data.append(float(data['gz']))
            fileLength += 1
    ...
    output_file = f"{input_file}_complementary.csv"  # Replace with your desired output file path
